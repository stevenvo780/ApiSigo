...
export const validateInvoice = [
  ...
  body('cliente.ruc').optional().isLength({ min: 11, max: 11 }).withMessage('RUC del cliente debe tener 11 dígitos'),
  body('cliente.nit').optional().isLength({ min: 9, max: 15 }).withMessage('NIT del cliente debe ser válido'),
...
export const validateCancelReason = [
  body('motivo').notEmpty().withMessage('Motivo de anulación es requerido')
    .isLength({ min: 10, max: 500 }).withMessage('Motivo debe tener entre 10 y 500 caracteres')
];
 ...
 * Crear factura
 */
export const createInvoice = async (req: InvoiceRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
 ...
 * Obtener factura
 */
export const getInvoice = async (req: InvoiceParamsRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({
 ...
 */
export const updateInvoiceStatus = async (req: StatusUpdateRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({
 ...
 */
export const sendInvoiceToSunat = async (req: InvoiceParamsRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({
 ...
 * Anular factura
 */
export const cancelInvoice = async (req: CancelInvoiceRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({
 ...
 */
export const getInvoiceStatus = async (req: InvoiceParamsRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
      res.status(400).json({
 ...
 * Reenviar factura
 */
export const resendInvoice = async (req: InvoiceParamsRequest, res: Response, next: NextFunction): Promise<void> => {
  try {
    const { serie, numero } = req.params;
